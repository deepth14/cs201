...
public class Driver {
	...
	public Driver(){
		...
		while(!inputString.equalsIgnoreCase("N")){
			...
			addEmployee(nu);
				...
				System.out.println(outputString);
				String foodI=sc.nextLine();
				checkWhetherFoodItemInMarket(foodI);
			}
	...
	void displayNumberOfEmployees() {
		...
		int x =directory.getNumberOfBookstoreEmployees();
		System.out.println("Number of employees is: " +x+" " +" (Look for other console messages.)"+line+CONTINUE);
	}

	   void addEmployee(String employeeId){
		//implement-- redo the implementation
		//call the corresponding method on StoreDirectory Class
		 boolean abe=  directory.addNewEmployee(employeeId);
		 if(abe){
	...
	void checkIfBookIsInStock(String bookId){
		//implement -- redo the implementation
		directory.bookIsInStock(bookId);
	...
	void checkWhetherFoodItemInMarket(String foodItem){
		//	implement-- redo the implementation
		boolean foodie=directory.marketCarriesFoodItem(foodItem);
		if(foodie){
			System.out.println("Yes the market carries "+foodItem+line+CONTINUE);
		}else
		System.out.println("no the market didn't carries"+foodItem);
	}
